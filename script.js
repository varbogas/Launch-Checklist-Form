...
window.addEventListener("load", function() {
   ...
   form.addEventListener("submit", function(event) {
      ...
      //Set field filled validators
      let allFieldsPassed = false;
      let pilotNamePassed = false; 
      let coPilotNamePassed = false; 
      let fuelLevelPassed = false;
      let cargoWeightPassed = false;
      ...
      if (isNaN(cargoWeightInput.value) === true) {
         ...
         cargoWeightPassed = true;
      }
 
   if (allFieldsPassed === true && pilotNamePassed === true && coPilotNamePassed === true && fuelLevelPassed === true && cargoWeightPassed === true){

      //Check for fuel and cargo levels
      if(fuelLevelInput.value <= 10000 || cargoWeightInput.value > 10000){
         ...
         event.preventDefault();
            ...
            if(fuelLevelInput.value <= 10000){
               ...
               fuelLevelDiv.innerHTML = "Fuel level high enough for launch";
               event.preventDefault();
            }

               if(cargoWeightInput.value > 10000){
                  cargoLevelDiv.innerHTML = "Cargo weight too high for launch";  
                  event.preventDefault();
               }
                  
               else {
                  cargoLevelDiv.innerHTML = "Cargo weight low enough for launch"; 
                  event.preventDefault();
               } 
